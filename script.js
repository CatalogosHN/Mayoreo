...
function formatLempiras(amount, decimals = 2) {
  const n = Number(amount);
  const safe = Number.isFinite(n) ? n : 0;
  return "L " + safe.toLocaleString("es-HN", {
...
function getSavedTheme() {
  ...
  return "light"; // ✅ por defecto: claro (como en tu diseño nuevo)
}
...
// === MAYOREO ===
// ✅ -20% desde 3 unidades del MISMO producto
const WHOLESALE_MIN_QTY = 3;
const WHOLESALE_DISCOUNT = 0.20;

function getWholesalePrice(basePrice) {
  const p = Number(basePrice);
  const safe = Number.isFinite(p) ? p : 0;
  return safe * (1 - WHOLESALE_DISCOUNT);
}

function isWholesaleQty(qty) {
  return Number(qty) >= WHOLESALE_MIN_QTY;
}

// Soporta: getEffectiveUnitPrice(item) o getEffectiveUnitPrice(price, qty)
function getEffectiveUnitPrice(itemOrPrice, qty) {
  const isItem = typeof itemOrPrice === "object" && itemOrPrice !== null;
  const price = isItem ? itemOrPrice.price : itemOrPrice;
  const q = isItem ? itemOrPrice.qty : qty;
  const base = Number(price);
  const safe = Number.isFinite(base) ? base : 0;
  return isWholesaleQty(q) ? getWholesalePrice(safe) : safe;
}
...
function renderProducts(list = products) {
  ...
  sortedList.forEach((p, i) => {
    ...
    // SKU: si algún día agregás p.sku, lo respeta.
    const skuNum = String(safeIndex + 1).padStart(6, "0");
    const sku = p.sku ? p.sku : skuNum;

    const cat = Array.isArray(p.categories) && p.categories.length ? p.categories[0] : "Variedades";
    const shortDesc = Array.isArray(p.description) && p.description.length ? p.description[0] : "";
    const wholesalePrice = getWholesalePrice(p.price);

    card.innerHTML = `
      <div class="product-media">
        <div class="slider slider-static" id="slider-${safeIndex}">
      ...
      <div class="product-body">
        ...
        <div class="meta-row">
          <span class="sku-pill">SKU: ${sku}</span>
        </div>
        ...
        <div class="price-box">
          <div class="price-main">${formatLempiras(p.price)} <span class="unit">c/u</span></div>
          <div class="price-wholesale">${formatLempiras(wholesalePrice)} <span class="wholesale-note">mayoreo (${WHOLESALE_MIN_QTY}+ pz)</span></div>
        </div>
      </div>

      <button class="add-btn add-btn-full" onclick="addToCart(${safeIndex})">
        <span class="add-icon">🛒</span> Añadir al carrito
      </button>
    `;
...
function updateCart() {
  ...
  cart.forEach((item, i) => {
    ...
    const unit = getEffectiveUnitPrice(item);                 // ✅ aplica mayoreo cuando qty >= 3
    const wholesaleOn = isWholesaleQty(item.qty);
    ...
    div.innerHTML = `
      <div class="cart-item-info">
        ...
        <p class="price">
          ${formatLempiras(unit)} c/u × ${item.qty}
          ${wholesaleOn ? `<span class="wholesale-badge">Mayoreo -${Math.round(WHOLESALE_DISCOUNT * 100)}%</span>` : ``}
        </p>
        ${wholesaleOn ? `<p class="old-price">Antes: ${formatLempiras(baseUnit)} c/u</p>` : ``}
      </div>
...
function getCartSubtotal(){
  try { 
    return cart.reduce((s,i)=> s + (getEffectiveUnitPrice(i) * (Number(i.qty)||0)), 0);
  } catch { 
    return 0; 
  }
}
...
checkoutForm.addEventListener("submit", (e) => {
  ...
  const itemsTxt = cart.map(i => {
    ...
    const wholesaleOn = isWholesaleQty(i.qty);
    return `- ${i.qty} × ${i.name} (${formatLempiras(unit)} c/u${wholesaleOn ? " — mayoreo" : ""})\n  ${link}`;
  }).join("\n");
